...
uint8_t linbusOff_FltCheck_Start_Flag = FALSE;
...
bool Contains_ONE_Fr_sllr(const uint8_t *array, uint8_t size)
{
	...
	{
	    if ((array[i] == (uint8_t)EVNT_STATUS_ERROR) || (array[i] == (uint8_t)EVNT_STATUS_ERROR_2)) 
			{
	        ret = true;
	    }
	}
...
{	
	...
	{
		linbusOff_FltCheck_Start_Flag = FALSE;
	...
	{
		if((flt_onoff == 1) && (Evnt_IGN2_Onoff == 1))
		{
			flt_onoff = 0;
