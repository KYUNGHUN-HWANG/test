...
void DTC_Read_Code(void)
{
	...
	for(int i=0; i< MAX_DTC_COUNT; i++)
	{
		...
		if(nv.nv_item.uds_dtc_nv.DtcRecord[i].dtcCode != 0x000000)
		{
			...
			//sprintf((char *)temp_data,"%02x%02x%02x", dtc_data.high_byte, dtc_data.middle_byte, dtc_data.low_byte);
			//strcat((char *)Dtc_data, (const char *)temp_data);
...
void DTC_Send_event()
{
	...
	switch(dtc_subFunc)
	{
		...
		{
			DTC_Read_Code();
			SendDTCMessage(tx_id, Dtc_data, strlen((char *)Dtc_data), dtc_serviceId, dtc_subFunc, dtcstatus);			
			break;
		}
		...
		case 0x08: //ReportDTCExtendedDataRecordByDTCNumber
		{
			memset(Tx_candata, 0xAA, sizeof(Tx_candata));
			Tx_candata[0] = 0x7F;
			Tx_candata[1] = 0x19;
...
void hexStringToBytes(const char *hexString, uint8_t *byteArray, int length)
{
	for (int i = 0; i < length; i++)
	{
		byteArray[i] = (asciiToHex(hexString[i * 2]) << 4) | asciiToHex(hexString[i * 2 + 1]);
	}
}
	...
	if((pci & 0xF0) == 0x00)
	{
		switch(dtc_subFunc)
		{
			...
			{					
				...
				fw_size = (((size_check[2] << 16) & 0xFF0000) | ((size_check[1] << 8) & 0x00FF00) | (size_check[0] & 0x0000FF));
				...
				else if(dtcstatus == 0x02)
				{
					...
					if((fw_sum == header_sum))
					{
						...
						Tx_Ota[2] = dtc_subFunc;
						Tx_Ota[3] = dtcstatus;
						Tx_Ota[4] = ((fw_sum & 0xFF00) >> 8);
						Tx_Ota[5] = fw_sum & 0xFF;
						SendCANData(24, tx_id, Tx_Ota, sizeof(Tx_Ota));
					}
					else
					{
			...
			{					
				...
				Tx_Ota[2] = dtc_subFunc;
				Tx_Ota[3] = dtcstatus;
				Tx_Ota[4] = ((Vatt_LimitValue & 0xFF00) >> 8);
				Tx_Ota[5] = Vatt_LimitValue & 0x00FF;
				Tx_Ota[6] = ((Ign2_AdcMinValue & 0xFF00) >> 8);
				Tx_Ota[7] = Ign2_AdcMinValue & 0x00FF; 
				SendCANData(24, tx_id, Tx_Ota, sizeof(Tx_Ota));
			...
			case 0x06:	//CAN DBC CHECK
			{
				...
				Tx_Ota[2] = dtc_subFunc;
				Tx_Ota[3] = dtcstatus;
				Tx_Ota[4] = (CAN_DBC_VER >> 16) & 0xFF;
				Tx_Ota[5] = (CAN_DBC_VER >> 8) & 0xFF;
				Tx_Ota[6] = CAN_DBC_VER & 0xFF;
			...
			{	
				...
				Tx_Ota[2] = dtc_subFunc;
				Tx_Ota[3] = dtcstatus;
				Tx_Ota[4] = (LIN_LDF_VER >> 16) & 0xFF;
				Tx_Ota[5] = (LIN_LDF_VER >> 8) & 0xFF;
				Tx_Ota[6] = LIN_LDF_VER & 0xFF;
			...
			{			
				...
				Amo_timer_Stop(timer_69);		
				Adc_Value();
				if((Ign2_AdcMinValue <= BATT_CAL_DATA) && (Vatt_LimitValue <= BATT_CAL_DATA))
				{
				...
				Tx_Ota[2] = dtc_subFunc;
				Tx_Ota[3] = dtcstatus;
				Tx_Ota[4] = ((batt_cal & 0xFF00) >> 8);
				Tx_Ota[5] = batt_cal & 0x00FF;
				Tx_Ota[6] = ((ign_cal & 0xFF00) >> 8);
				Tx_Ota[7] = ign_cal & 0x00FF;
...
void DTC_OTA_event(void)
{
		...
		if(dtc_serviceId == 0x34)
		{
			switch(dtc_subFunc)
			{
				...
				case 0x03:	
				{
					...
					memset(temp_data, 0x00, sizeof(temp_data));
					memcpy(&temp_data[0], &Rx_candata[0], 8);
					...
					if(ret)
					{
						...
						Tx_Ota[1] = dtc_serviceId+0x40;
						Tx_Ota[2] = dtc_subFunc;
						Tx_Ota[3] = temp_data[3];
						Tx_Ota[4] = temp_data[4];
						Tx_Ota[5] = temp_data[5];
						Tx_Ota[6] = temp_data[6];
						Tx_Ota[7] = P_type;
						SendCANData(24, tx_id, Tx_Ota, sizeof(Tx_Ota));			
					}
					else
					{
						...
						//memset(Tx_candata, 0xAA, sizeof(Tx_candata));
						//Tx_candata[0] = (0x0 << 4) | (dtc_data_length+4);
						Tx_Ota[0] = 0x7F;
		...
		if(dtc_serviceId == 0x37)
		{
			switch(dtc_subFunc)
			{
				case 0x01: //CRC CHECK & FW Write
				{
					...
					#endif
					//uint32_t crc_received = ((recvBuff2.data[3] << 24) | (recvBuff2.data[4] << 16) | (recvBuff2.data[5] << 8) | recvBuff2.data[6]);
					uint16_t crc_received = (Rx_candata[3] << 8) | Rx_candata[4]; //

					//crc32_calculated = calculate_crc32(Write_Buffer, sn_index);
					...
					{
						//memset(Tx_candata, 0xAA, sizeof(Tx_candata));
						//Tx_candata[0] = (0x0 << 4) | dtc_data_length;
						Tx_Ota[0] = 0x7F;
						Tx_Ota[1] = 0x37;				
						Tx_Ota[2] = 0x01;	
					...
					{
						INT_SYS_DisableIRQGlobal();
						//ret = FLASH_DRV_EraseSector(&flashSSDConfig, FLASH_HEADER_ADDRESS_2, FLASH_SECTOR_SIZE);
						//DEV_ASSERT(STATUS_SUCCESS == ret);
						...
						//memset(Tx_candata, 0xAA, sizeof(Tx_candata));
						//printf("crc_received = %4x \r\n", crc_received, chksum_data, sn_index, recvBuff2.data[3], recvBuff2.data[4]);
						Tx_Ota[0] = (0x0 << 4) | 6;
						Tx_Ota[1] = dtc_serviceId+0x40;
						Tx_Ota[2] = dtc_subFunc;
						Tx_Ota[3] = Rx_candata[3];
						Tx_Ota[4] = Rx_candata[4];
						//Tx_Ota[5] = recvBuff2.data[5];
						//Tx_Ota[6] = recvBuff2.data[6];
				...
				case 0x02: //FW UPDATE DONE
				{
					...
					if(flash_address == FLASH_FW1_ADDRESS_1)
					{
						...
						#endif
						INT_SYS_DisableIRQGlobal();
						ret = FLASH_DRV_EraseSector(&flashSSDConfig, new_header_address, FLASH_SECTOR_SIZE);
						//DEV_ASSERT(STATUS_SUCCESS == ret);
						ret = FLASH_DRV_Program(&flashSSDConfig, new_header_address, FLASH_SECTOR_SIZE, (uint8_t *)&Fw_header_test);
						//DEV_ASSERT(STATUS_SUCCESS == ret);
						INT_SYS_EnableIRQGlobal();		
						...
						//printf("%02x %02x %02x %02x \r\n", Fw_header_test.version[0], Fw_header_test.version[1], Fw_header_test.version[2], Fw_header_test.version[3]);

						INT_SYS_DisableIRQGlobal();
						ret = FLASH_DRV_EraseSector(&flashSSDConfig, old_header_address, FLASH_SECTOR_SIZE);
						//DEV_ASSERT(STATUS_SUCCESS == ret);
						ret = FLASH_DRV_Program(&flashSSDConfig, old_header_address, FLASH_SECTOR_SIZE, (uint8_t *)&Fw_header_test);
						//DEV_ASSERT(STATUS_SUCCESS == ret);
						INT_SYS_EnableIRQGlobal();		
	...
	#endif
	else if(((pci & 0xF0) >> 4) == 0x01) //multi frame 0x36
	{
		dtc_serviceId = Rx_candata[2];
		dtc_subFunc = Rx_candata[3];		

		dtc_Total_length = ((((pci & 0x0F) << 8) & 0xFF00) | (Rx_candata[1] & 0x00FF));
		...
		{
			Tx_Ota[0] = (0x1 << 4) | ((dtc_Total_length >> 8) & 0x000F);
			Tx_Ota[1] = (dtc_Total_length & 0x00FF);				
					   ...
					   uint32_t buffIdx, flexcan_state_t *flexcanState)
{
	...
	//							Evt_queue_add(DEVICE_ACTUATOR_RECOVERY_EVENT);
						...
						{
							if(Datc_onoff && ATCU_OperationStatus == 0x01)
							{
								Datc_onoff = 0;
							...
							if((Vent_Mode_FrDrOnOff) || (Vent_Mode_FrPassOnOff))
							{
								if((Hu_Fr_dr_Vent_Center_signal_invalid != 0x03) || (Hu_Fr_dr_Vent_Side_signal_invalid != 0x03) || (Hu_Fr_Ps_Vent_Center_signal_invalid != 0x03) || (Hu_Fr_Ps_Vent_Side_signal_invalid != 0x03))
								{
									if((((!Hu_Fr_dr_Vent_Center_signal_success) && (!Hu_Fr_dr_Vent_Side_signal_success)) && (!Hu_Fr_Ps_Vent_Center_signal_success)) && (!Hu_Fr_Ps_Vent_Side_signal_success))
									{
										...
										{
											if(!Hu_Fr_dr_Vent_Center_signal_success)
											{
										...
										{
											if(!Hu_Fr_dr_Vent_Side_signal_success)
											{
										...
										{
											if(!Hu_Fr_Ps_Vent_Center_signal_success)
											{
										...
										{
											if(!Hu_Fr_Ps_Vent_Side_signal_success)
											{
			...
			{		
				switch(recvBuff2.msgId)
				{
					case 0x505:		
					{
						...
						if(((pci & 0xF0) >> 4) == 0x01) //multi frame
						{
							Evt_queue_add(DEVICE_EVNT_OTA_EVENT); 
						}					
...
void Last_Setting_Save(Profile_mode Profile_mode)
{
	...
	if(Profile_mode !=0)
	{
		nv.nv_item.profile_item[Profile_mode].items.fr_dr_mode = Hu_Fr_Dr_Vent_Mode;
		nv.nv_item.profile_item[Profile_mode].items.fr_ps_mode = Hu_Fr_Ps_Vent_Mode; 
#ifdef AMO_GN7_PE_SETTING_NONE
		nv.nv_item.profile_item[Profile_mode].items.rear_dr_mode = Hu_Rr_Dr_Vent_Mode;
		nv.nv_item.profile_item[Profile_mode].rear_ps_mode = Hu_Rr_Ps_Vent_Mode;
#endif
		nv.nv_item.profile_item[Profile_mode].items.fr_dr_side_opncls = Hu_Fr_dr_Vent_Side_signal_backup;
		nv.nv_item.profile_item[Profile_mode].items.fr_dr_ctr_opncls = Hu_Fr_dr_Vent_Center_signal_backup;
		nv.nv_item.profile_item[Profile_mode].items.fr_ps_side_opncls = Hu_Fr_Ps_Vent_Side_signal_backup;
		nv.nv_item.profile_item[Profile_mode].items.fr_ps_ctr_opncls = Hu_Fr_Ps_Vent_Center_signal_backup;
#ifdef AMO_GN7_PE_SETTING_NONE
		nv.nv_item.profile_item[Profile_mode].items.rr_dr_side_opncls = Hu_Rr_Dr_Vent_signal;	
		nv.nv_item.profile_item[Profile_mode].items.rr_ps_ctr_opncls = Hu_Rr_Ps_Vent_signal; 
#endif			
		nv.nv_item.profile_item[Profile_mode].items.fr_dr_side_leftright_target = Fr_Dr_side_leftright_target_backup;
		nv.nv_item.profile_item[Profile_mode].items.fr_dr_side_updown_target = Fr_Dr_side_updown_target_backup;
		nv.nv_item.profile_item[Profile_mode].items.fr_dr_ctr_leftright_target = Fr_Dr_ctr_leftright_target_backup;
		nv.nv_item.profile_item[Profile_mode].items.fr_dr_ctr_updown_target = Fr_Dr_ctr_updown_target_backup;
		nv.nv_item.profile_item[Profile_mode].items.fr_ps_side_leftright_target = Fr_Ps_side_leftright_target_backup;
		nv.nv_item.profile_item[Profile_mode].items.fr_ps_side_updown_target = Fr_Ps_side_updown_target_backup;
		nv.nv_item.profile_item[Profile_mode].items.fr_ps_ctr_leftright_target = Fr_Ps_ctr_leftright_target_backup;
		nv.nv_item.profile_item[Profile_mode].items.fr_ps_ctr_updown_target = Fr_Ps_ctr_updown_target_backup;
#ifdef AMO_GN7_PE_SETTING_NONE
		nv.nv_item.profile_item[Profile_mode].items.rr_dr_side_leftright_target = Rr_Dr_side_leftright_target_backup;
		nv.nv_item.profile_item[Profile_mode].items.rr_dr_side_updown_target = Rr_Dr_side_updown_targe_backup;
		nv.nv_item.profile_item[Profile_mode].items.rr_ps_ctr_leftright_target = Rr_Ps_ctr_leftright_target_backup;
		nv.nv_item.profile_item[Profile_mode].items.rr_ps_ctr_updown_target = Rr_Ps_ctr_updown_target_backup;
#endif
		...
		{
		nv.nv_item.profile_item[Profile_mode].items.hu_fr_dr_sidept_x = Hu_FrDrEVntSidePt_X_backup;
		nv.nv_item.profile_item[Profile_mode].items.hu_fr_dr_sidept_y = Hu_FrDrEVntSidePt_Y_backup;
		nv.nv_item.profile_item[Profile_mode].items.hu_fr_dr_ctrpt_x = Hu_FrDrEVntCtrPt_X_backup;
		nv.nv_item.profile_item[Profile_mode].items.hu_fr_dr_ctrpt_y = Hu_FrDrEVntCtrPt_Y_backup;

		nv.nv_item.profile_item[Profile_mode].items.hu_fr_ps_sidept_x = Hu_FrPsEVntSidePt_X_backup;
		nv.nv_item.profile_item[Profile_mode].items.hu_fr_ps_sidept_y = Hu_FrPsEVntSidePt_Y_backup;
		nv.nv_item.profile_item[Profile_mode].items.hu_fr_ps_ctrpt_x = Hu_FrPsEVntCtrPt_X_backup;
		nv.nv_item.profile_item[Profile_mode].items.hu_fr_ps_ctrpt_y = Hu_FrPsEVntCtrPt_Y_backup;
		}
		else if(Hu_Fr_Dr_Vent_Mode == FREE_MODE || Hu_Fr_Ps_Vent_Mode == FREE_MODE)
		{
		nv.nv_item.profile_item[Profile_mode].items.hu_fr_dr_sidept_x = Hu_FrDrEVntSidePt_X;
		nv.nv_item.profile_item[Profile_mode].items.hu_fr_dr_sidept_y = Hu_FrDrEVntSidePt_Y;
		nv.nv_item.profile_item[Profile_mode].items.hu_fr_dr_ctrpt_x = Hu_FrDrEVntCtrPt_X;
		nv.nv_item.profile_item[Profile_mode].items.hu_fr_dr_ctrpt_y = Hu_FrDrEVntCtrPt_Y;

		nv.nv_item.profile_item[Profile_mode].items.hu_fr_ps_sidept_x = Hu_FrPsEVntSidePt_X;
		nv.nv_item.profile_item[Profile_mode].items.hu_fr_ps_sidept_y = Hu_FrPsEVntSidePt_Y;
		nv.nv_item.profile_item[Profile_mode].items.hu_fr_ps_ctrpt_x = Hu_FrPsEVntCtrPt_X;
		nv.nv_item.profile_item[Profile_mode].items.hu_fr_ps_ctrpt_y = Hu_FrPsEVntCtrPt_Y;			
		}
		else
		{
		nv.nv_item.profile_item[Profile_mode].items.hu_fr_dr_sidept_x = Hu_FrDrEVntSidePt_X_backup;
...
#ifdef AMO_GN7_PE_SETTING_NONE
		nv.nv_item.profile_item[Profile_mode].items.hu_rr_dr_constpt_x = Hu_RrDrEVntConsPt_X;
		nv.nv_item.profile_item[Profile_mode].items.hu_rr_dr_constpt_y = Hu_RrDrEVntConsPt_Y;
		nv.nv_item.profile_item[Profile_mode].items.hu_rr_ps_constpt_x = Hu_RrPsEVntConsPt_X;
		nv.nv_item.profile_item[Profile_mode].items.hu_rr_ps_constpt_x = Hu_RrPsEVntConsPt_Y;			
#endif
		nv.nv_item.write_cnt = nv.nv_item.write_cnt + 1;
		nv_max_cnt = nv.nv_item.write_cnt;
		nv.nv_item.nv_checksum = Calculate_Chksum(nv.nv_data, 441);
...
void Last_Setting_Setup(Profile_mode Profile_mode)
{
	...
	Hu_Fr_Dr_Vent_Mode = nv.nv_item.profile_item[Profile_mode].items.fr_dr_mode;
	Hu_Fr_Dr_Vent_Mode_Remember = Hu_Fr_Dr_Vent_Mode;
	Hu_Fr_Ps_Vent_Mode = nv.nv_item.profile_item[Profile_mode].items.fr_ps_mode;
	Hu_Fr_Ps_Vent_Mode_Remember = Hu_Fr_Ps_Vent_Mode;
#ifdef AMO_GN7_PE_SETTING_NONE			
	Hu_Rr_Dr_Vent_Mode = nv.nv_item.profile_item[Profile_mode].items.rear_dr_mode;
	Hu_Rr_Ps_Vent_Mode = nv.nv_item.profile_item[Profile_mode].items.rear_ps_mode;
#endif		
	Hu_Fr_dr_Vent_Side_signal_backup = nv.nv_item.profile_item[Profile_mode].items.fr_dr_side_opncls;
	Hu_Fr_dr_Vent_Center_signal_backup = nv.nv_item.profile_item[Profile_mode].items.fr_dr_ctr_opncls;
	Hu_Fr_Ps_Vent_Side_signal_backup = nv.nv_item.profile_item[Profile_mode].items.fr_ps_side_opncls;
	Hu_Fr_Ps_Vent_Center_signal_backup = nv.nv_item.profile_item[Profile_mode].items.fr_ps_ctr_opncls;
#ifdef AMO_GN7_PE_SETTING_NONE
	Hu_Rr_Dr_Vent_signal = nv.nv_item.profile_item[Profile_mode].items.rr_dr_side_opncls; 
	Hu_Rr_Ps_Vent_signal = nv.nv_item.profile_item[Profile_mode].items.rr_ps_ctr_opncls; 
#endif			
	Fr_Dr_side_leftright_target_backup = nv.nv_item.profile_item[Profile_mode].items.fr_dr_side_leftright_target;
	Fr_Dr_side_updown_target_backup = nv.nv_item.profile_item[Profile_mode].items.fr_dr_side_updown_target;
	Fr_Dr_ctr_leftright_target_backup = nv.nv_item.profile_item[Profile_mode].items.fr_dr_ctr_leftright_target;
	Fr_Dr_ctr_updown_target_backup = nv.nv_item.profile_item[Profile_mode].items.fr_dr_ctr_updown_target;
	Fr_Ps_side_leftright_target_backup = nv.nv_item.profile_item[Profile_mode].items.fr_ps_side_leftright_target;
	Fr_Ps_side_updown_target_backup = nv.nv_item.profile_item[Profile_mode].items.fr_ps_side_updown_target;
	Fr_Ps_ctr_leftright_target_backup = nv.nv_item.profile_item[Profile_mode].items.fr_ps_ctr_leftright_target;
	Fr_Ps_ctr_updown_target_backup = nv.nv_item.profile_item[Profile_mode].items.fr_ps_ctr_updown_target;
#ifdef AMO_GN7_PE_SETTING_NONE			
	Rr_Dr_side_leftright_target_backup = nv.nv_item.profile_item[Profile_mode].items.rr_dr_side_leftright_target;
	Rr_Dr_side_updown_targe_backup = nv.nv_item.profile_item[Profile_mode].items.rr_dr_side_updown_target;
	Rr_Ps_ctr_leftright_target_backup = nv.nv_item.profile_item[Profile_mode].items.rr_ps_ctr_leftright_target;
	Rr_Ps_ctr_updown_target_backup = nv.nv_item.profile_item[Profile_mode].items.rr_ps_ctr_updown_target;
#endif			
	Hu_FrDrEVntSidePt_X_backup = nv.nv_item.profile_item[Profile_mode].items.hu_fr_dr_sidept_x;
	Hu_FrDrEVntSidePt_Y_backup = nv.nv_item.profile_item[Profile_mode].items.hu_fr_dr_sidept_y;
	Hu_FrDrEVntCtrPt_X_backup = nv.nv_item.profile_item[Profile_mode].items.hu_fr_dr_ctrpt_x;
	Hu_FrDrEVntCtrPt_Y_backup = nv.nv_item.profile_item[Profile_mode].items.hu_fr_dr_ctrpt_y;
	Hu_FrPsEVntSidePt_X_backup = nv.nv_item.profile_item[Profile_mode].items.hu_fr_ps_sidept_x;
	Hu_FrPsEVntSidePt_Y_backup = nv.nv_item.profile_item[Profile_mode].items.hu_fr_ps_sidept_y;
	Hu_FrPsEVntCtrPt_X_backup = nv.nv_item.profile_item[Profile_mode].items.hu_fr_ps_ctrpt_x;
	Hu_FrPsEVntCtrPt_Y_backup = nv.nv_item.profile_item[Profile_mode].items.hu_fr_ps_ctrpt_y;
#ifdef AMO_GN7_PE_SETTING_NONE			
	Hu_RrDrEVntConsPt_X_backup = nv.nv_item.profile_item[Profile_mode].items.hu_rr_dr_constpt_x;
	Hu_RrDrEVntConsPt_Y_backup = nv.nv_item.profile_item[Profile_mode].items.hu_rr_dr_constpt_y;
	Hu_RrPsEVntConsPt_X_backup = nv.nv_item.profile_item[Profile_mode].items.hu_rr_ps_constpt_x;
	Hu_RrPsEVntConsPt_Y_backup = nv.nv_item.profile_item[Profile_mode].items.hu_rr_ps_constpt_x;
#endif			
...
void Default_Setting_Setup(Profile_mode Profile_mode)
{
	...
	Hu_Rr_Ps_Vent_Mode = nv.nv_item.profile_item[Profile_mode].items.rear_ps_mode;
#endif			
	Hu_Fr_dr_Vent_Side_signal = nv.nv_item.profile_item[Profile_mode].items.fr_dr_side_opncls;
	Hu_Fr_dr_Vent_Center_signal = nv.nv_item.profile_item[Profile_mode].items.fr_dr_ctr_opncls;
	Hu_Fr_Ps_Vent_Side_signal = nv.nv_item.profile_item[Profile_mode].items.fr_ps_side_opncls;
	Hu_Fr_Ps_Vent_Center_signal = nv.nv_item.profile_item[Profile_mode].items.fr_ps_ctr_opncls;
#ifdef AMO_GN7_PE_SETTING_NONE
	Hu_Rr_Dr_Vent_signal = nv.nv_item.profile_item[Profile_mode].items.rr_dr_side_opncls; 
	Hu_Rr_Ps_Vent_signal = nv.nv_item.profile_item[Profile_mode].items.rr_ps_ctr_opncls; 
#endif			
...
void flexcan0_ErrorCallback(uint8_t instance, flexcan_event_type_t eventType, flexcan_state_t *flexcanState)
{
	...
	CAN_Type * const g_flexcanBase1[] = CAN_BASE_PTRS;
	...
	switch(eventType)
	{
		case FLEXCAN_EVENT_ERROR:
		{
			...
			if(error & CAN_ESR1_BOFFINT_MASK) //bus off error
			{
			...
			if(error & CAN_ESR1_TXWRN_MASK) //transmit error
			{
			...
			if(error & CAN_ESR1_RXWRN_MASK) //receive error
			{
			...
			if(error & CAN_ESR1_STFERR_MASK) //stuff error
			{
			...
			if(error & CAN_ESR1_FLTCONF(2))  //error passive
			{				
			...
			if((error & CAN_ESR1_FLTCONF(3)) != 0)  //bus off error
			{				
			...
			if(error & CAN_ESR1_CRCERR_MASK)  //crc error
			{				
			...
			if(error & CAN_ESR1_ACKERR_MASK)  //Acknowledge error
			{				
			...
			if(error & CAN_ESR1_FRMERR_MASK)  //Acknowledge error
			{				
			...
			if(error & CAN_ESR1_BIT0ERR_MASK)  //bit error
			{				
			...
			if(error & CAN_ESR1_BIT1ERR_MASK)  //bit error
			{				
