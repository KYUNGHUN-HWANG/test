...
{	
	if(lin_Status->fr_cllr_status.LDATA.FR_CLLR_Steploss == 1)
	{
		steplossCount_1_cllr++; //50ms=1 , 100ms=2, 150ms=3, 200ms=4, 250ms=5, 300ms=6
	...
	if((lin_Status->fr_cllr_status.LDATA.FR_CLLR_Steploss == 0) && (recoveryModeFlag_cllr == TRUE)) //go To Target
	{
		if(lin_Status->fr_cllr_status.LDATA.FR_CLLR_OpDone == true)
		{
			zero_flag_cllr = TRUE;
			recoveryModeFlag_cllr = FALSE;				
			...
			Amo_timer_Stop(timer_94);
			Amo_timer_Start(timer_94, 300, false, GO_To_TargetPt_cllr);
		}
	}
}
...
{	
	if(lin_Status->fr_clud_status.LDATA.FR_CLUD_Steploss == 1)
	{
		steplossCount_1_clud++; //50ms=1 , 100ms=2, 150ms=3, 200ms=4, 250ms=5, 300ms=6
	...
	if((lin_Status->fr_clud_status.LDATA.FR_CLUD_Steploss == 0) && (recoveryModeFlag_clud == TRUE)) //go To Target
	{
		if(lin_Status->fr_clud_status.LDATA.FR_CLUD_OpDone == true)
		{
			zero_flag_clud = TRUE;
			recoveryModeFlag_clud = FALSE;		
			...
			Amo_timer_Stop(timer_97);
			Amo_timer_Start(timer_97, 300, false, GO_To_TargetPt_clud);
		}
	}
}
...
{	
	...
	if((lin_Status->fr_sllr_status.LDATA.FR_SLLR_Steploss == 0) && (recoveryModeFlag_sllr == TRUE)) //go To Target
	{
		if(lin_Status->fr_sllr_status.LDATA.FR_SLLR_OpDone == true)
		{
			zero_flag_sllr = TRUE;
			recoveryModeFlag_sllr = FALSE;				
			...
			Amo_timer_Stop(timer_112);
			Amo_timer_Start(timer_112, 300, false, GO_To_TargetPt_sllr);
		}
	}
}
		...
		if((eventCountTest_sllr == 0) && (recoveryFinishFlag_sllr == FALSE))
		{
			...
			{	
				...
				{				
					...
					Amo_timer_Stop(timer_100);
					Amo_timer_Start(timer_100, 300, false, Check_300ms_5_sllr);
				}
			}
		}
...
void SetFunc_secureMainFlag(void)
{
	FR_SLLR_FailSafe_Flag = TRUE;
	FR_SLUD_FailSafe_Flag = TRUE;
	FR_CLLR_FailSafe_Flag = TRUE;
...
void SetFunc_rxRecoveryMainFlag(void)
{
	FR_SLLR_FailSafe_Flag = FALSE;
	FR_SLUD_FailSafe_Flag = FALSE;
	FR_CLLR_FailSafe_Flag = FALSE;
