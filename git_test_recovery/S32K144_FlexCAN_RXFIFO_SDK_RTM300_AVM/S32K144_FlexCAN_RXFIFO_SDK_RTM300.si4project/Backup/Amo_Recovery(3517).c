...
//        moved = (uint16_t)abs((int32_t)target - (int32_t)state->prev_position);
				...
				moved = (lin_Pt->FrDrPt.targetPt) - (state->prev_position);
				abs_moved = absValue(moved);
				
				if(abs_moved > 3000)
				{
					if(lin_Pt->FrDrPt.targetPt > 3000)
					{
						lin_Pt->FrDrPt.targetPt = 4096-(lin_Pt->FrDrPt.targetPt);
					}
					...
					abs_moved = (lin_Pt->FrDrPt.targetPt) + (state->prev_position);
				}
