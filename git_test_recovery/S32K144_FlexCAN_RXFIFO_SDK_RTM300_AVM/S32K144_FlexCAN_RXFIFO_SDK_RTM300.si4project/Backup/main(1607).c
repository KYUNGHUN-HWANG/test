...
uint8_t sleepFlag = 0;
uint8_t button_idx_1 = 0;
uint8_t button_idx_2 = 0;
...
int main(void)
{
	...
	while(1)
	{
		...
		switch(Board_state)
		{
			...
			{
				//Last mode action			
				printf("DEVICE_STATE_LAST_MAIN = %d \r\n", last_mode_save);
				switch(last_mode_save)
				{
					#if 0
					case DEFAULT:
					{	
						Last_Setting_Setup(DEFAULT);
						break;
					}
					#endif
					case GUEST:
					{
						nvmem_read(NVMEM_PROFILE_ACTIVE_IDX, &nv);

						if(nv.profile_active[GUEST] != 0)
						{
					...
					case PROFILE_1:
					{
						...
						if(nv.profile_active[PROFILE_1] != 0)
						{
					...
					case PROFILE_2:
					{
						...
						if(nv.profile_active[PROFILE_2] != 0)
						{
					...
					case PROFILE_3:
					{
						...
						if(nv.profile_active[PROFILE_3] != 0)
						{
					...
					case PROFILE_4:
					{
						...
						if(nv.profile_active[PROFILE_4] != 0)
						{
					...
					case PROFILE_5:
					{
						...
						if(nv.profile_active[PROFILE_5] != 0)
						{
					...
					case PROFILE_6:
					{
						...
						if(nv.profile_active[PROFILE_6] != 0)
						{
					...
					case PROFILE_7:
					{
						...
						if(nv.profile_active[PROFILE_7] != 0)
						{
					...
					case PROFILE_8:
					{
						...
						if(nv.profile_active[PROFILE_8] != 0)
						{
					...
					case PROFILE_9:
					{
						...
						if(nv.profile_active[PROFILE_9] != 0)
			{
...
//				sleepFlag = 1;
				...
				if(P_Oip_Cts_Select == P_OIP_SELECT)
				{
					if((Hu_Fr_Dr_Vent_Mode == FREE_MODE) && (Touch_mode_Dr_set == HU_DRIVER_TOUCH))
					{
						...
						nv.profile = last_mode_save;
						nvmem_write(NVMEM_PROFILE_ID_IDX,&nv,1);

						nv.profile_active[last_mode_save] = last_mode_save;
						nvmem_write(NVMEM_PROFILE_ACTIVE_IDX,&nv,11);					
