...
void recovery_mode_check(SlaveId_Flag_t slaveId_flag)
{
	...
	{
		recovery_mode_step(SLAVE_SLLR, 10);
	}
	if(slaveId_flag & SLAVE_SLUD_FLAG)
	{
		recovery_mode_step(SLAVE_SLUD, 10);
	}
}
...
void recovery_mode_step(SlaveId_t id, uint8_t rpm)
{
    ...
    if (id < NUM_SLAVES)
    {
        ...
        {
            if ((opdone == true) && ((state->curr_position >= 0 && state->curr_position < 20) || (state->curr_position >= 4096 && state->curr_position < 4076)))
            {
