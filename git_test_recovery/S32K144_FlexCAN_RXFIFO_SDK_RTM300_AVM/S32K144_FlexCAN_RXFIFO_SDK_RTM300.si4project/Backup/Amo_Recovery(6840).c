	...
	//            state->target_position = 0U;
	            ...
	            send_move_command(id, 0U, &LIN_LH_EVNT_MASTER_CMD, &LIN_RH_EVNT_MASTER_CMD);
							...
							if(state->target_position < 3000U)
							{
								...
								else //state->prev_position >= 3000U
								{
									if(act_state > (state->prev_position))
									{
										state->cant_reach_zeroPt = true;
...
void process_slave_command(SlaveId_t id, Lin_EVntPt_t *lin_Pt, uint8_t rpm)
{
    ...
    uint32_t timeout;
		uint16_t act_state;
		uint16_t cal_target_pt = 0;
		uint16_t cal_prev_pt = 0;
...
//        moved = (uint16_t)abs((int32_t)target - (int32_t)state->prev_position);
				...
				if(abs_moved > 3000U)
				{
					if(state->target_position > 3000U)
					{
						cal_target_pt = 4096U-(state->target_position);
					}
					if(state->prev_position > 3000U)
					{
						cal_prev_pt = 4096U-(state->prev_position);
					}
					
					abs_moved = cal_target_pt + cal_prev_pt;
				}
        ...
        if (state->in_recovery)
        {
		        ...
		        send_move_command(id, state->target_position, &LIN_LH_EVNT_MASTER_CMD, &LIN_RH_EVNT_MASTER_CMD);
						...
						if(abs_moved > 3000U)
						{
							...
							{
								cal_target_pt = state->target_position;
							}
							if(state->curr_position > 3000U)
							{
								cal_prev_pt = 4096U-(state->curr_position);
							}
							else
							{
								cal_prev_pt = state->curr_position;
							}
		        ...
		        /* If your timer API supports passing an argument, pass 'id' to the callback */
					...
					{
            if ((opdone == true) && ((state->curr_position >= 0U && state->curr_position < 30U) || (state->curr_position >= 4096U && state->curr_position < 4066U)))
            {
