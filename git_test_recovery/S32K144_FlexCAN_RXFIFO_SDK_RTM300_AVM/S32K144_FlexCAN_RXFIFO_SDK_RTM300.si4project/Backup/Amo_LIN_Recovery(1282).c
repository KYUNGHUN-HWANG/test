...
uint8_t recoveryFailCount_sllr = 0;
uint8_t recoveryFailCount_slud = 0;
uint8_t recoveryFailCount_cllr = 0;
uint8_t recoveryFailCount_clud = 0;
uint8_t recoveryFailCount_crlr = 0;
uint8_t recoveryFailCount_crud = 0;
uint8_t recoveryFailCount_srlr = 0;
uint8_t recoveryFailCount_srud = 0;
...
//            FR_PS_ExtFlt_SET();
            ...
            recoveryFlags.FR_SLLR_Recov_Flag = FALSE;
						...
						recoveryFailCount_sllr = 0;

						if((Hu_Fr_Dr_Vent_Mode == SWING_MODE) && (FrDrPsCycleMode_Flag == TRUE))
						{
							FrDrPsCycleMode_Flag = FALSE;
							
							FrPsCycleMode_Flag = TRUE;
							Recovery_CycleMode_ReStart();
						}
						
        }

				if((recoveryFlags.FR_SLUD_Recov_Flag == FALSE) && (recoveryFlags.FR_CLLR_Recov_Flag == FALSE) && (recoveryFlags.FR_CLUD_Recov_Flag == FALSE))
				{
	        Amo_timer_Stop(timer_62);
...
//            FR_PS_ExtFlt_SET();
            ...
            recoveryFlags.FR_SLUD_Recov_Flag = FALSE;
				...
				if((recoveryFlags.FR_CLLR_Recov_Flag == FALSE) && (recoveryFlags.FR_CLUD_Recov_Flag == FALSE))
				{
	        Amo_timer_Stop(timer_62);
	        Amo_timer_Start(timer_62, 50, false, Lin_LH_Scheduler_Normal_Start_1); //20 //50 //500
				}
...
//			Hu_Fr_Dr_Vent_Mode = DEFAULT_MODE;
		...
		if(recoveryFlags.FR_CLUD_Recov_Flag == FALSE)
		{
			Amo_timer_Stop(timer_62);
			Amo_timer_Start(timer_62, 50, false, Lin_LH_Scheduler_Normal_Start_1); //20 //50 //500
		}
...
//			Hu_Fr_Dr_Vent_Mode = DEFAULT_MODE;
//			Hu_Fr_dr_Vent_Center_signal_backup = 0x03;
...
//            FR_PS_ExtFlt_SET();
            ...
            recoveryFlags.FR_CRLR_Recov_Flag = FALSE;
						...
						recoveryFailCount_crlr = 0;

						if((Hu_Fr_Ps_Vent_Mode == SWING_MODE) && (FrDrPsCycleMode_Flag == TRUE))
						{
							FrDrPsCycleMode_Flag = FALSE;
				...
				if((recoveryFlags.FR_CRUD_Recov_Flag == FALSE) && (recoveryFlags.FR_SRLR_Recov_Flag == FALSE) && (recoveryFlags.FR_SRUD_Recov_Flag == FALSE))
				{
	        Amo_timer_Stop(timer_124);
...
//            FR_PS_ExtFlt_SET();
            ...
            recoveryFlags.FR_CRUD_Recov_Flag = FALSE;
				...
				if((recoveryFlags.FR_SRLR_Recov_Flag == FALSE) && (recoveryFlags.FR_SRUD_Recov_Flag == FALSE))
				{
	        Amo_timer_Stop(timer_124);
	        Amo_timer_Start(timer_124, 50, false, Lin_RH_Scheduler_Normal_Start_1); //20 //50 //500
				}
...
//            FR_PS_ExtFlt_SET();
            ...
            recoveryFlags.FR_SRLR_Recov_Flag = FALSE;
				...
				if(recoveryFlags.FR_SRUD_Recov_Flag == FALSE)
				{
	        Amo_timer_Stop(timer_124);
	        Amo_timer_Start(timer_124, 50, false, Lin_RH_Scheduler_Normal_Start_1); //20 //50 //500
				}
...
//	if(zero_flag_cllr == TRUE && eventCountTest_cllr >= 1)
	...
	//				zero_flag = 0;
			...
			if((steplossCount_1_cllr >= 20) && (Check300ms_flag_1_cllr == TRUE)) //abnormal
			{
				Check300ms_flag_1_cllr = FALSE;
				ClearState_Steploss_OpDone_Check_cllr();
			}
		}
		else if((lin_Status->fr_cllr_status.LDATA.FR_CLLR_Steploss == 0) && (lin_Status->fr_cllr_status.LDATA.FR_CLLR_OpDone == 1)) //recovery Clear
		{
...
//	if(zero_flag_cllr == TRUE && eventCountTest_cllr >= 1)
	...
	//				zero_flag = 0;
			...
			if((steplossCount_1_clud >= 20) && (Check300ms_flag_1_clud == TRUE)) //abnormal
			{
				Check300ms_flag_1_clud = FALSE;
				ClearState_Steploss_OpDone_Check_clud();
			}
		}
		else if((lin_Status->fr_clud_status.LDATA.FR_CLUD_Steploss == 0) && (lin_Status->fr_clud_status.LDATA.FR_CLUD_OpDone == 1)) //recovery Clear
		{
...
//	if(zero_flag_cllr == TRUE && eventCountTest_cllr >= 1)
	...
	//				zero_flag = 0;
			...
			if((steplossCount_1_sllr >= 20) && (Check300ms_flag_1_sllr == TRUE)) //abnormal
			{
				Check300ms_flag_1_sllr = FALSE;
				ClearState_Steploss_OpDone_Check_sllr();
			}
		}
		else if((lin_Status->fr_sllr_status.LDATA.FR_SLLR_Steploss == 0) && (lin_Status->fr_sllr_status.LDATA.FR_SLLR_OpDone == 1)) //recovery Clear
		{
...
//	if(zero_flag_cllr == TRUE && eventCountTest_cllr >= 1)
	...
	//				zero_flag = 0;
			...
			if((steplossCount_1_slud >= 20) && (Check300ms_flag_1_slud == TRUE)) //abnormal
			{
				Check300ms_flag_1_slud = FALSE;
				ClearState_Steploss_OpDone_Check_slud();
			}
		}
		else if((lin_Status->fr_slud_status.LDATA.FR_SLUD_Steploss == 0) && (lin_Status->fr_slud_status.LDATA.FR_SLUD_OpDone == 1)) //recovery Clear
		{
...
//	if(zero_flag_cllr == TRUE && eventCountTest_cllr >= 1)
	...
	//				zero_flag = 0;
			...
			if((steplossCount_1_crlr >= 20) && (Check300ms_flag_1_crlr == TRUE)) //abnormal
			{
				Check300ms_flag_1_crlr = FALSE;
				ClearState_Steploss_OpDone_Check_crlr();
			}
...
//	if(zero_flag_cllr == TRUE && eventCountTest_cllr >= 1)
	...
	//				zero_flag = 0;
			...
			if((steplossCount_1_crud >= 20) && (Check300ms_flag_1_crud == TRUE)) //abnormal
			{
				Check300ms_flag_1_crud = FALSE;
				ClearState_Steploss_OpDone_Check_crud();
			}
		}
		else if((lin_Status->fr_crud_status.LDATA.FR_CRUD_Steploss == 0) && (lin_Status->fr_crud_status.LDATA.FR_CRUD_OpDone == 1)) //recovery Clear
		{
...
//	if(zero_flag_cllr == TRUE && eventCountTest_cllr >= 1)
	...
	//				zero_flag = 0;
			...
			if((steplossCount_1_srlr >= 20) && (Check300ms_flag_1_srlr == TRUE)) //abnormal
			{
				Check300ms_flag_1_srlr = FALSE;
				ClearState_Steploss_OpDone_Check_srlr();
			}
		}
		else if((lin_Status->fr_srlr_status.LDATA.FR_SRLR_Steploss == 0) && (lin_Status->fr_srlr_status.LDATA.FR_SRLR_OpDone == 1)) //recovery Clear
		{
...
//	if(zero_flag_cllr == TRUE && eventCountTest_cllr >= 1)
	...
	//				zero_flag = 0;
			...
			if((steplossCount_1_srud >= 20) && (Check300ms_flag_1_srud == TRUE)) //abnormal
			{
				Check300ms_flag_1_srud = FALSE;
				ClearState_Steploss_OpDone_Check_srud();
			}
		}
		else if((lin_Status->fr_srud_status.LDATA.FR_SRUD_Steploss == 0) && (lin_Status->fr_srud_status.LDATA.FR_SRUD_OpDone == 1)) //recovery Clear
		{
...
void Steploss_check_cllr(t_lin_LH_FR_STATUS *lin_Status, Lin_EVntPt_t *lin_Pt)
{
	...
	if(l_flg_tst_LI0_FR_CLLR_Steploss_flag())
	{
		...
		if((eventCountTest_cllr != 0) && (recoveryPattern_cllr == Recovery_None))
		{
			recoveryFailCount_cllr++;

			if(recoveryFailCount_cllr == 1)
			{
				Recovery_Clear_FlagCountReset_cllr();
...
void Steploss_check_clud(t_lin_LH_FR_STATUS *lin_Status, Lin_EVntPt_t *lin_Pt)
{
	...
	if(l_flg_tst_LI0_FR_CLUD_Steploss_flag())
	{
		...
		if((eventCountTest_clud != 0) && (recoveryPattern_clud == Recovery_None))
		{
			recoveryFailCount_clud++;

			if(recoveryFailCount_clud == 1)
			{
				Recovery_Clear_FlagCountReset_clud();
...
void Steploss_check_sllr(t_lin_LH_FR_STATUS *lin_Status, Lin_EVntPt_t *lin_Pt)
{
	...
	if(l_flg_tst_LI0_FR_SLLR_Steploss_flag())
	{
		...
		if((eventCountTest_sllr != 0) && (recoveryPattern_sllr == Recovery_None))
		{
			recoveryFailCount_sllr++;

			if(recoveryFailCount_sllr == 1)
			{
				Recovery_Clear_FlagCountReset_sllr();
...
void Steploss_check_slud(t_lin_LH_FR_STATUS *lin_Status, Lin_EVntPt_t *lin_Pt)
{
	...
	if(l_flg_tst_LI0_FR_SLUD_Steploss_flag())
	{
		...
		{
			recoveryFailCount_slud++;

			if(recoveryFailCount_slud == 1)
			{
				Recovery_Clear_FlagCountReset_slud();
...
void Steploss_check_crlr(t_lin_RH_FR_STATUS *lin_Status, Lin_EVntPt_t *lin_Pt)
{
	...
	if(l_flg_tst_LI1_FR_CRLR_Steploss_flag())
	{
		...
		if((eventCountTest_crlr != 0) && (recoveryPattern_crlr == Recovery_None))
		{
			recoveryFailCount_crlr++;

			if(recoveryFailCount_crlr == 1)
			{
				Recovery_Clear_FlagCountReset_crlr();
...
void Steploss_check_crud(t_lin_RH_FR_STATUS *lin_Status, Lin_EVntPt_t *lin_Pt)
{
	...
	if(l_flg_tst_LI1_FR_CRUD_Steploss_flag())
	{
		...
		if((eventCountTest_crud != 0) && (recoveryPattern_crud == Recovery_None))
		{
			recoveryFailCount_crud++;

			if(recoveryFailCount_crud == 1)
			{
				Recovery_Clear_FlagCountReset_crud();
...
void Steploss_check_srlr(t_lin_RH_FR_STATUS *lin_Status, Lin_EVntPt_t *lin_Pt)
{
	...
	if(l_flg_tst_LI1_FR_SRLR_Steploss_flag())
	{
		...
		if((eventCountTest_srlr != 0) && (recoveryPattern_srlr == Recovery_None))
		{
			recoveryFailCount_srlr++;

			if(recoveryFailCount_srlr == 1)
			{
				Recovery_Clear_FlagCountReset_srlr();
...
void Steploss_check_srud(t_lin_RH_FR_STATUS *lin_Status, Lin_EVntPt_t *lin_Pt)
{
	...
	if(l_flg_tst_LI1_FR_SRUD_Steploss_flag())
	{
		...
		if((eventCountTest_srud != 0) && (recoveryPattern_srud == Recovery_None))
		{
			recoveryFailCount_srud++;

			if(recoveryFailCount_srud == 1)
			{
				Recovery_Clear_FlagCountReset_srud();
